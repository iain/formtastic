...
Customize the input element by specifying a block. The input element that would have been rendered is passed as an argument. Do with it whatever you like. Wrap it in another tag, mutate it, or discard it completely. The label, hint and error message will not be touched.

<pre>
  <% semantic_form_for @post do |form| %>
    <% form.input :title do |html| %>
      <%= content_tag(:span, html, :class => "important") %>
    <% end %>
    <%= form.input :body %>
  <% end %>
</pre>
